...
4. **Write strategy — `str_replace` is highest priority.**
...
5. Per-platform write strategy:
   - **Lark docx** — `docs +fetch --section <heading>` to read the current section, then `docs +update` with `str_replace` targeted at the exact section text. Use `block_insert_after` only for NEW sections/blocks. Use `block_replace` only after `str_replace` fails and the user explicitly confirms that block replacement is acceptable. **Never use `overwrite`** on the whole doc.
