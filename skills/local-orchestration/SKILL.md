...
Bare `mcursor` opens the workspace session picker. Selection and new-session actions replace `mcursor` with Cursor's native persistent TUI; `mcursor` does not wrap the running TUI.
