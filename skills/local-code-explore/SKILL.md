...
## Explore phase
- Resolve the exploration scope before searching. When invoked inside `<root>/local_workspaces/<workspace>/` or one of its repositories, treat that workspace folder as the active workspace and `<root>` as the root workspace.
- Search repositories in this order: the active workspace, direct repositories under `<root>`, then `<root>/_external/` when a required call-chain target is external. If a required repository is absent from the active workspace, continue exploration from its root-workspace checkout instead of stopping or reporting it missing.
- Prefer the active-workspace checkout when the same repository exists in both locations because it reflects the workspace branch. Explore only one checkout of a repository in a single graph; do not duplicate evidence from its root-workspace checkout.
- Do not recursively scan unrelated workspaces under `<root>/local_workspaces/`. Enter another workspace only when the user explicitly names it; the normal fallback for a repository missing from the active workspace is its direct checkout under `<root>`.
- Treat `<root>/_external/` as a read-only external repository index. Invoking this skill authorizes inspecting a matching external repository when it is required to follow a proven RPC, HTTP, message, or other code dependency. Do not scan unrelated external repositories. Label evidence from `_external/<repo>` as external context, never as editable implementation.
...
- If an explored node comes from `_external/`, mark it as `external: yes` inside the service box and include only evidence required by the active call chain.
