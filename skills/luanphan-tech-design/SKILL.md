...
- **IGNORE workspace folders** when scanning for microservice codebases: any subfolder containing a `workspace.yml` at its top level is a multi-repo git-worktree workspace (created via the `luanphan-workspace` skill / `mkws`) and its contents are duplicates of sibling repos already in the root. Skip both the workspace folder and everything inside it; only consider the original sibling repos as candidates for the mapping.
