...
test_meta_hub() {
	...
	init_repo "$root/station-a/_external/example-context"
	init_repo "$root/station-b/repo-b"
	add_origin_remote "$root/station-a/repo-a" "$TMP/repo-a.git"
	add_origin_remote "$root/station-a/_external/example-context" "$TMP/example-context.git"
	...
	assert_contains "$TMP/meta-index-a.out" "=== external index ==="
	assert_contains "$TMP/meta-index-a.out" "_external/example-context"
	...
	assert_exists "$root/station-a/local_workspaces/feature-a/tech_doc/.git"
	...
	assert_contains "$clone/station-a/workstation.yml" "path: \"_external/example-context\""
...
EOF
	...
	printf 'stale local copy\n' > "$restore_root/station-a/local_workspaces/feature-a/testing.md"
	...
	assert_exists "$restore_root/station-a/_external/example-context/.git"
	...
	assert_contains "$TMP/meta-pull-restore.out" "_external/example-context"
	assert_contains "$TMP/meta-pull-restore.out" "=== home histories ==="
	assert_exists "$restore_root/station-a/local_workspaces/feature-a/tech_doc/.git"
	assert_exists "$restore_root/station-a/local_workspaces/feature-a/tech_doc/notes.md"
	assert_contains "$restore_root/station-a/local_workspaces/feature-a/testing.md" "workspace test notes"
	assert_not_contains "$restore_root/station-a/local_workspaces/feature-a/testing.md" "stale local copy"
	assert_contains "$restore_root/station-a/local_workspaces/feature-a/tech_doc/stale.md" "stale"
	assert_contains "$restore_root/station-a/local_workspaces/feature-a/local-only.md" "local only"
